...
const log = logger.child({ pkg: 'metricsCollector' });
...
export class PrometheusMetricsCollector {
  ...
  constructor(name: string, opts: MetricsCollectorOptions) {
    ...
    this.name = name;
    this.bullmqOpts = opts.bullmqOpts ?? { connection: { maxRetriesPerRequest: null } };
    ...
    if (opts.queues) {
      ...
      this.discoverAllQueues()
        ...
        .catch((err) => {
          log.error(`Failed to discover queues: ${err}`);
          process.exit(125);
        });
    }
  }
  ...
  async discoverAllQueues() {
    const keyPattern = new RegExp(`^${this.bullmqOpts.prefix}:([^:]+):(id|failed|active|waiting|stalled-check)$`);
  ...
  async close() {
    log.debug('Closing metrics collector');
    try {
      const val = await this.defaultRedisClient.quit();
      log.debug('Successfully quit redis connection', 'response', val);
    } catch (e: unknown) {
      log.warn('Failed to quit redis connection', 'error', e);
    }
